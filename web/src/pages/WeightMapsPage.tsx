        ...
        {viewMode === "heatmap" ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Card className="lg:col-span-2 p-4">
              ...
              {isWeightsError ? (
                <div className="p-6 bg-surface border border-hazard-alert/30 rounded-lg text-center space-y-2">
                  <div className="flex items-center justify-center gap-2 text-hazard-alert font-semibold text-xs">
                    <AlertCircle className="w-4 h-4" />
                    <span>Unable to load weight matrix.</span>
                  </div>
                  <p className="text-[11px] text-text-muted">
                    {(weightsError as Error)?.message || "A network or server error occurred while retrieving weights."}
                  </p>
                </div>
              ) : weightsLoading ? (
                <div className="py-12">
